...
function myWrapperFunction() {
  const SearchStepTwo: React.FC = () => {
    ...
    function enviarRequisicao() {
      ...
      const urlResumo = urlGeral + `/originals_words?initials=${pesquisaInputFormatado}&type=ABSTRACT`;
    ...
    return (
      <div className=' m-[0 auto] w-full '>
        ...
        <div ref={dropdownRef} className='group w-full transition ' onChange={() => setIsOpen(isOpen)} onFocus={() => setIsOpen(isOpen)} >
          <div className='flex flex-wrap md:flex-nowrap top-6 w-full border-b-[1px] border-gray-300'>

            <div className='flex w-full flex-col'>
              <div className={`flex   items-center h-14 group w-full  text-base font-medium  justify-center transition  ${botaoTermosClicado ? 'hover:border-[#005399]' : ''} ${botaoResumoClicado ? 'hover:border-red-400' : ''} ${botaoAreasClicado ? 'hover:border-[#8FC53E]' : ''} ${botaoPesquisadoresClicado ? 'hover:border-[#20BDBE]' : ''} ${isOpen && pesquisaInput.length >= 3 ? 'rounded-tl-3xl' : 'rounded-l-full'}`}>
                ...
                <input
                  ...
                  id="" className="w-full h-full outline-none bg-[#f9f9f9]" />
              </div>

            </div>

            <Tabs>
              <TabList className={`  p-2 flex gap-2   border-l-0
            ${isOpen && pesquisaInput.length >= 3 ? 'rounded-tr-3xl' : 'rounded-r-full'}`}>
                ...
                <Tab selected={selectedTab === 3} className={` outline-none cursor-pointer text-sm text-gray-400 rounded-full flex items-center gap-2  px-4 py-2 font-semibold  transition ${botaoPesquisadoresClicado ? "activePesquisadores" : ('')}`} onClick={handleClickPesquisadores} >
                  <IdentificationCard size={16} className="text-gray-400" />
                  Nome
                </Tab>


                <div className='flex items-center justify-center'>
                  <div className={`absolute z[-999] animate-ping gap-4 text-white rounded-full h-[28px] w-[28px] justify-center hover:bg-blue-500  font-medium transition ${botaoTermosClicado ? 'bg-[#005399]' : ''} ${botaoAreasClicado ? 'bg-[#8FC53E]' : ''} ${botaoResumoClicado ? 'bg-[#EC1C22]' : ''} ${botaoPesquisadoresClicado ? 'bg-[#20BDBE]' : ''}`}></div>
                  <div onClick={handleClick} className={`cursor-pointer flex z-[999] relative  items-center gap-4 text-white rounded-full h-[38px] w-[38px] justify-center  font-medium transition ${botaoTermosClicado ? 'bg-[#005399]' : ''} ${botaoResumoClicado ? 'bg-[#EC1C22]' : ''} ${botaoAreasClicado ? 'bg-[#8FC53E]' : ''} ${botaoPesquisadoresClicado ? 'bg-[#20BDBE]' : ''}`}><Funnel size={16} className="text-white" /></div>
                </div>
          ...
          {pesquisaInput.length <= 2 ? (
            ...
            <div id='teste'
              className={`flex-col relative mt-6`}
            >
